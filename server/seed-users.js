...
const seedUsers = async () => {
  try {
    ...
    await User.deleteMany({ email: { $in: ['admin@rakshasetu.in', 'divya@rakshasetu.in', 'control@rakshasetu.in', 'responder@rakshasetu.in'] } });
    ...
    // 4. Create Control Unit User
    const controlPassword = await bcrypt.hash('control123', 10);
    const controlUser = new User({
      name: 'Central Control Hub',
      email: 'control@rakshasetu.in',
      phone: '6666666666',
      password: controlPassword,
      role: 'control'
    });

    // 5. Create Field Responder User
    const fieldPassword = await bcrypt.hash('responder123', 10);
    const fieldUser = new User({
      name: 'Ambulance Unit 4',
      email: 'responder@rakshasetu.in',
      phone: '5555555555',
      password: fieldPassword,
      role: 'field'
    });

    await User.insertMany([adminUser, standardUser, pharmUser, controlUser, fieldUser]);
    ...
    console.log('Control Unit:  control@rakshasetu.in / control123');
    console.log('Field Team:    responder@rakshasetu.in / responder123');
    console.log('---------------------------');
