...
exports.searchMedicines = async (req, res) => {
  ...
  try {
    ...
    if (!medicine) {
      // Logic for alternatives remains the same... (see below)
    }

    // 2. Find pharmacies with this medicine available
    let query = {
      'inventory.medicine': medicine ? medicine._id : null,
      'inventory.status': { $ne: 'Out of Stock' }
    };
    
    // Optional: Add isOnline check if your model supports it
    // query.isOpen = true; 

    let pharmacies = await Pharmacy.find(query).populate('inventory.medicine');
    ...
    let results = pharmacies.map(ph => {
      ...
      const item = ph.inventory.find(i => 
        i.medicine && (i.medicine._id.toString() === (medicine ? medicine._id.toString() : ''))
      );
