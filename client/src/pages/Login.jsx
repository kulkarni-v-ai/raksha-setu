  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      if (loginMethod === 'password') {
        ...
        const res = await fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/auth/login-password', {
          method: 'POST',
        ...
        } else if (data.user.role === 'control') {
          navigate('/responder/control');
        } else if (data.user.role === 'field') {
          navigate('/responder/field');
        } else {
          navigate('/dashboard');
        }
      } else {
        ...
        if (!loginOtpSent) {
          ...
          const res = await fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/auth/send-otp', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          const res = await fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/auth/login-otp', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            ...
            {view === 'register' && (
              <form onSubmit={handleRegisterSubmit} className="auth-form fade-in">
                ...
                {!regOtpSent && !regOtpVerified && (
                  <div className="role-selector" style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                    <div 
                      className={`role-option glass ${regRole === 'user' ? 'active' : ''}`}
                      onClick={() => setRegRole('user')}
                      style={{ flex: '1 1 30%', padding: '0.75rem', borderRadius: '0.75rem', textAlign: 'center', cursor: 'pointer', border: regRole === 'user' ? '2px solid var(--primary)' : '1px solid var(--glass-border)' }}
                    >
                      <User size={18} style={{ marginBottom: '0.25rem' }} />
                      <div style={{ fontSize: '0.75rem', fontWeight: 600 }}>User</div>
                    </div>
                    <div 
                      className={`role-option glass ${regRole === 'pharmacy' ? 'active' : ''}`}
                      onClick={() => setRegRole('pharmacy')}
                      style={{ flex: '1 1 30%', padding: '0.75rem', borderRadius: '0.75rem', textAlign: 'center', cursor: 'pointer', border: regRole === 'pharmacy' ? '2px solid var(--primary)' : '1px solid var(--glass-border)' }}
                    >
                      <ShoppingCart size={18} style={{ marginBottom: '0.25rem' }} />
                      <div style={{ fontSize: '0.75rem', fontWeight: 600 }}>Pharmacy</div>
                    </div>
                    <div 
                      className={`role-option glass ${regRole === 'field' ? 'active' : ''}`}
                      onClick={() => setRegRole('field')}
                      style={{ flex: '1 1 30%', padding: '0.75rem', borderRadius: '0.75rem', textAlign: 'center', cursor: 'pointer', border: regRole === 'field' ? '2px solid var(--primary)' : '1px solid var(--glass-border)' }}
                    >
                      <Ambulance size={18} style={{ marginBottom: '0.25rem' }} />
                      <div style={{ fontSize: '0.75rem', fontWeight: 600 }}>Responder</div>
                    </div>
                  </div>
                )}
