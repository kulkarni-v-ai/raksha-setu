...
const MedicineManager = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [medRes, demandRes] = await Promise.all([
        fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/pharmacy/medicines', { headers }),
        fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/pharmacy/demands', { headers })
      ]);
  ...
  const handleInlineQty = async (name, newQty) => {
    ...
    try {
      ...
      await fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/pharmacy/medicines', {
        method: 'POST',
  ...
  const handleBulkStatus = async (newStatus) => {
    ...
    try {
      ...
      await Promise.all(targets.map(m => 
        fetch((import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:5000')) + '/api/pharmacy/medicines', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleDelete = async (name) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/pharmacy/medicines/${name}`, {
        method: 'DELETE',
