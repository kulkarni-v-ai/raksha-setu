...
const SOSMonitoring = () => {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const active = data.filter(a => a.status === 'active' || a.status === 'en_route');
      const history = data.filter(a => a.status === 'resolved');
  ...
  const handleContactUser = (phone) => {
    if (phone && phone !== 'N/A') {
      window.location.href = `tel:${phone}`;
    } else {
      alert('Contact number not available for this user.');
    }
  };

  const filteredAlerts = (activeTab === 'active' ? alerts : historyAlerts).filter(a => 
  ...
  useEffect(() => {
    ...
    alerts.forEach(alert => {
      ...
      if (!markersRef.current[alertId]) {
        const color = alert.status === 'active' || alert.status === 'en_route' ? '#ef4444' : '#10b981';
        ...
        markersRef.current[alertId] = marker;
      }
    });
  }, [alerts]);
  ...
  const focusAlert = (alert) => {
    ...
    if (mapInstance.current && alert.location) {
      const coords = alert.location.coordinates;
      mapInstance.current.flyTo([coords[1], coords[0]], 15, {
        duration: 1.5,
        easeLinearity: 0.25
      });
      ...
      setTimeout(() => {
        if (markersRef.current[alert._id]) {
            markersRef.current[alert._id].bringToFront();
        }
      }, 100);
    }
  };
  ...
  const resolveAlert = async (id) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/admin/sos/${id}/resolve`, {
        method: 'PUT'
      });
      if (res.ok) {
    ...
    <div className="sos-monitoring animate-slide-up">
      <div className="module-header">
        ...
        <div className="header-status-pills">
           ...
           <span className="txt">System Live: {alerts.length} Active Alerts</span>
        </div>
      </div>

      <div className="search-bar-inline" style={{ marginBottom: '1.5rem', width: '100%', maxWidth: '360px' }}>
      ...
      <div className="sos-grid">
        ...
        <div className="sos-feed-column">
          <div className="feed-tabs">
            <button 
              className={`feed-tab ${activeTab === 'active' ? 'active' : ''}`}
              onClick={() => setActiveTab('active')}
            >
              Active ({alerts.length})
            </button>
            <button 
              className={`feed-tab ${activeTab === 'history' ? 'active' : ''}`}
          ...
          <div className="alerts-scroll-area">
            {loading ? <div style={{ padding: '2rem', textAlign: 'center' }}>Loading alerts...</div> : 
             filteredAlerts.map((alert) => (
              <div 
                key={alert._id} 
                className={`sos-alert-card glassmorphism ${selectedAlert?._id === alert._id ? 'active' : ''}`}
              ...
              >
                 ...
                 <div className="card-row">
                    <div className="p-avatar" style={{ width: '48px', height: '48px', fontSize: '1.2rem' }}>{alert.userId?.name.charAt(0) || 'U'}</div>
                    <div className="card-main">
                      <h4 className="user-name" style={{ marginBottom: '0.15rem' }}>{alert.userId?.name || 'Anonymous User'}</h4>
                      <div className="meta-item"><Clock size={12} /> {new Date(alert.createdAt).toLocaleTimeString()}</div>
                      <div className="meta-item"><MapPin size={12} /> {alert.address || 'Location Hidden'}</div>
                    </div>
                    <ChevronRight size={18} className="arrow-icon" style={{ opacity: selectedAlert?._id === alert._id ? 1 : 0.3 }} />
                 </div>
              </div>
            ))}
          </div>
              ...
              <div className="map-detail-overlay animate-slide-up" style={{ zIndex: 1000 }}>
                <div className="overlay-header">
                  <div className="oh-info">
                    <h3 style={{ margin: 0, fontSize: '1.4rem' }}>{selectedAlert.userId?.name || 'User'}</h3>
                ...
                <div className="overlay-body">
                  <div className="data-bit">
                    ...
                    <p style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><MapPin size={14} color="var(--primary)" /> {selectedAlert.address || 'Fetching address...'}</p>
                  </div>
                  <div className="data-bit">
                    <label>Assigned Responder</label>
                    <p style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Navigation size={14} color={selectedAlert.responderId ? "var(--success)" : "var(--warning)"} /> 
                      {selectedAlert.responderId ? "Responder Dispatched" : "Pending Assignment"}
                    </p>
                  </div>
                  <div className="dispatch-actions">
                    <button className="admin-primary-btn phone" style={{ flex: 1, background: 'var(--card-bg)', border: '1px solid var(--primary)', color: 'var(--primary)', boxShadow: 'none' }} onClick={() => handleContactUser(selectedAlert.userId?.phone)}>
                      <Phone size={16} /> Contact User
                    </button>
                    {selectedAlert.status !== 'resolved' && (
                      <button className="admin-primary-btn resolve" style={{ flex: 1 }} onClick={() => resolveAlert(selectedAlert._id)}>
